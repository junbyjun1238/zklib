...
theorem add_zero (spec : CommRingLaws) (a : spec.Carrier) :
    spec.add a spec.zero = a := by
  calc
    spec.add a spec.zero = spec.add spec.zero a := spec.add_comm a spec.zero
    _ = a := spec.zero_add a

theorem add_right_neg (spec : CommRingLaws) (a : spec.Carrier) :
...
theorem mul_one (spec : CommRingLaws) (a : spec.Carrier) :
    spec.mul a spec.one = a := by
  calc
    spec.mul a spec.one = spec.mul spec.one a := spec.mul_comm a spec.one
    _ = a := spec.one_mul a

theorem right_distrib (spec : CommRingLaws) (a b c : spec.Carrier) :
...
theorem add_left_cancel (spec : CommRingLaws) {a b c : spec.Carrier}
...
theorem add_right_cancel (spec : CommRingLaws) {a b c : spec.Carrier}
